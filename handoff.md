...
**แยกปลายทางตามเหตุการณ์** (Version 15 ขึ้นไป)
...
## งานที่ค้างอยู่ (15 ก.ย. 2026)

**แยกกลุ่ม "อนุมัติครบ" ยังไม่เสร็จ** — ติดที่ฝั่ง LINE ไม่ใช่โค้ด

- groupId ของกลุ่มใหม่**เก็บได้แล้ว**: `Ca52dcb375c6da830eaa9a4000abfe6d3`
  (ระบบคว้าได้จาก event `join` ตอนบอทเข้ากลุ่ม ก่อนมันจะเด้งออก 2 วินาทีต่อมา)
- **ปัญหา: บอทเข้ากลุ่มใหม่แล้วออกเองทันที** ทั้งที่ LINE OA Manager ตั้ง
  "อนุญาตให้เข้าร่วมกลุ่มหรือแชทแบบหลายคน" ไว้ถูกต้องแล้ว (ยืนยันด้วยภาพหน้าจอ)
  กลุ่มแรกเข้าได้ปกติ กลุ่มใหม่เข้าไม่ได้ — ยังหาสาเหตุไม่เจอ
  สงสัยว่า LINE ใช้เวลาให้ค่าที่เปลี่ยนมีผล ยังไม่ได้ทดสอบซ้ำหลังเว้นระยะ
- **ถอย `LINE_TO_APPROVED` ออกจาก Script Properties แล้ว** เพราะถ้าชี้กลุ่มที่บอทไม่ได้อยู่
  ข้อความ "อนุมัติครบ" จะส่งไม่ถึงแล้วหายเงียบ ตอนนี้ทุกข้อความเข้ากลุ่มหลักครบ

**ทำต่อยังไง** — พอบอทอยู่ในกลุ่มใหม่ได้ ใส่ groupId ข้างบนกลับเข้า `LINE_TO_APPROVED`
ได้เลย ไม่ต้องทำขั้นตอนเก็บใหม่

**โค้ดที่บันทึกไว้แต่ยัง deploy ไม่ผ่าน** — เพิ่มการตรวจรูปแบบปลายทางก่อนส่ง
(`/^[CUR][0-9a-f]{32}$/` ถ้าไม่ตรงให้ถอยไปกลุ่มหลัก) เพื่อให้ใส่ข้อความ placeholder
ใน `LINE_TO_APPROVED` ได้โดยไม่พัง — โค้ดอยู่ใน editor แล้ว **deployment ยังเป็น Version 16**
ต้อง Deploy → จัดการการทำให้ใช้งานได้ → แก้ตัวเดิม → New version อีกรอบ
