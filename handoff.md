...
**แต่ Apps Script ฝั่งอ่านยังไม่ส่งช่องนี้มา** — ตอนนี้ส่งมาแค่ 6 ช่องต่อสินค้า
(Product, Specification, Quantity Request, Shipment Request, Idea Price, %Margin (Idea))
ราคาเสนอรายชิ้นจึงถูกทิ้งตั้งแต่ฝั่ง server คอลัมน์นี้จะขึ้น `—` จนกว่าจะแก้ Apps Script
ให้ใส่ `ราคาเสนอ (USD/MT)` กับ `%Margin (Offer)` ลงใน products[] ด้วย
